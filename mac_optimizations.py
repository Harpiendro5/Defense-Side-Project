...
def mac_terminal():
	...
	print(f'{ROYAL_BLUE}This tool is designed for Windows, you are using this on mac and may be limited due to OS Restrictions.{RESET}')
	time.sleep(0.65)
...
load - Show system load average{RESET}""")
			...
			if option == "cpu temp":
				try:
					temp = subprocess.getoutput("osx-cpu-temp")
					print(f"CPU Temperature (C): {temp}")
					print(f"CPU Temperature (F): {round((float(temp.split('°C')[0]) * 9/5) + 32, 2)}°F")
				except Exception as e:
					print(f"{RED}Failed to get CPU temperature: {e}{RESET}")
